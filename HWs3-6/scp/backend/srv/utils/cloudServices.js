...
const BV_uaa = xsenv.getServices({
    xsuaa: {
        tag: "xsuaa"
    }
}).xsuaa;

const BV_conn = xsenv.getServices({
    conn: {
        tag: "connectivity"
    }
}).conn;

const BV_dest = xsenv.getServices({
    dest: {
        tag: "destination"
    }
}).dest;

const sUaaUrl = BV_uaa.url;

const sConnProxy = `http://${bv_conn.onpremise_proxy_host}:${bv_conn.onpremise_proxy_port}`;
const sConnSecret = `${bv_conn.clientid}:${bv_conn.clientsecret}`;
const sConnCredentials = Buffer.from(sConnSecret).toString('base64');

const sDestUrl = bv_dest.url;
const sDestSecret = `${bv_dest.clientid}:${bv_dest.clientsecret}`;
...
const getOnPremiseSystems = async () => {
    const sDestAccessToken = (await getUaaToken(sDestCredentials, bv_dest.clientid)).access_token;

    const oRequestOptions = {
        url: `${bv_dest.uri}/destination-configuration/v1/subaccountDestinations`,
        headers: {
...
const getOnPremiseSystemById = async systemId => {
    ...
    const oRequestOptions = {
        url: `${bv_dest.uri}/destination-configuration/v1/destinations/${systemId}`,
        headers: {
            'Authorization': 'Bearer ' + sDestAccessToken
        }
    };
...
const getOnPremiseSystemData = async (onPremiseSystem, sLang) => {
    ...
    const sConnAccessToken = (await getUaaToken(sConnCredentials, bv_conn.clientid)).access_token;
